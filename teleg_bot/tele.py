    ...
    if message.chat.id in chspis:
        ...
        for i in m:
            keyboard.add(telebot.types.InlineKeyboardButton(
             text="{}".format(i), callback_data="{}".format(i)))
        no_place = telebot.types.InlineKeyboardButton(
         text="Места нет в списке", callback_data='none')
        keyboard.add(no_place)
        bot.send_message(message.chat.id, '''Это места,
        где есть зарегестрированные волонтеры''', reply_markup=keyboard)
...
def gorod(call):
    if call.data == "none":
        bot.edit_message_text(chat_id=call.message.chat.id,
                              ...
                              text='Обратитесь за помощью - @noprazd')
    else:
        conn = MySQLdb.connect('БД vol')
        conn.set_character_set('utf8')
        cursor = conn.cursor()
        cursor.execute('''select name, mob,
         teleg from  info where place =\'{}\''''.format(call.data))
        otv = cursor.fetchall()
        cursor.execute('''select id from
         info where place =\'{}\''''.format(call.data))
...
def variant(message):
    ...
    if message.text != w[0]:
        ...
        connect.commit()
        connect.close()
        bot.send_message(message.chat.id, '''Мы внесли Вас в базу,
         Вы уже можете получить помощь''', reply_markup=mark)

    else:
        ud_vo = bot.send_message(message.chat.id, '''Напишите свои контактные
        данные в формате:
